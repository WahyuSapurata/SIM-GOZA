            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <select name="metode_bayar" class="form-select" data-control="select2" id="metode_bayar"
                            data-placeholder="Pilih jenis inputan">
                        </select>
                        <small class="text-danger metode_bayar_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Material</label>
                        <select name="material" class="form-select" data-control="select2" id="material_select"
                            data-placeholder="Pilih jenis inputan">
                        </select>
                        <small class="text-danger material_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Mobil</label>
                        <select name="mobil" class="form-select" data-control="select2" id="mobil_select"
                            data-placeholder="Pilih jenis inputan">
                        </select>
                        <small class="text-danger mobil_error"></small>
                    </div>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Penjualan');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/add-penjualan', 'Tambah',
                    'Penjualan',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/update-penjualan/' + uuid, 'Update',
                    'Penjualan', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/show-penjualan/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Penjualan', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/delete-penjualan/' + $(this).attr('data-uuid');
        ...
        let columns = [{
            ...
            className: 'text-center',
            render: function(data, type, row, meta) {
                const value = numeral(row.harga).format(
        ...
        let columnDefs = [{
            ...
            render: function(data, type, full, meta) {
                ...
                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Penjualan" class="btn btn-danger button-delete btn-icon btn-sm ${full.role === "{{ $role }}" ? 'disabled-link' : ''}">
        ...
        $(function() {
            control.push_select3(metodaBayar, '#metode_bayar');
            control.push_select3(material, '#material_select');
            control.push_select3(mobil, '#mobil_select');
            control.initDatatable('/admin/get-penjualan', columns, columnDefs);
        })
    </script>
@endsection
