...
@section('script')
    <script>
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/add-datauser', 'Tambah',
                    'User',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/update-datauser/' + uuid, 'Update',
                    'User', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/show-datauser/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'User', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/delete-datauser/' + $(this).attr('data-uuid');
        ...
        let columnDefs = [{
            ...
            render: function(data, type, full, meta) {
                ...
                <a href="javascript:;" type="button" data-uuid="${data}" data-label="User" class="btn btn-danger button-delete btn-icon btn-sm ${full.role === "{{ $role }}" ? 'disabled-link' : ''}">
        ...
        $(function() {
            ...
            control.initDatatable('/admin/get-datauser', columns, columnDefs);
        })
    </script>
@endsection
